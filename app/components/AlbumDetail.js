...
export const artistName = album => {
  return album.artists.map(artist => (
    <Text key={artist.name} style={{ marginLeft: 5 }}>
      {artist.name}
    </Text>
  ));
};
...
const AlbumDetail = ({ album }) => {
  ...
  const { headerContentStyle, headerTextStyle, buttonTextStyle } = styles;
    ...
    <Card>
      ...
      <CardSection>
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Text style={buttonTextStyle}>{I18n.t("preview")}</Text>
        </TouchableOpacity>
      </CardSection>
    </Card>
  );
